...
In the image below, an example can be seen where node \textit{q1} is not conformant to the original specification. This is resolved by introducing \textit{\_0} from which the edge that was originally going from \textit{q1} to \textit{q2} gets reattached.
...
\subsection{Continuous one counter automata reachability}
In this section we will give a description of the solution that was used to determine the reachability for continuous one counter automata. This will be done by showing the approach that was taken, as well as an application to show its usage. This approach is a practical implementation of the theoretical approach described by Michael Blondin et al. \cite{blondin2021continuous}.
...
\begin{landscape}
	\begin{figure}
		...
		\begin{tikzpicture}[auto, >=latex, font=\footnotesize\scshape]
			...
			\node[center]	at (0, 0) 		(q0) 	{Intersect \\input: x,y \\output: z};
			
			\node[center]	at (3, 6) 		(q1) 	{x is empty};
			\node[center]	at (3, 5)		(q2)	{y is empty};
			\node[center]	at (3, 2)		(q5)	{$x_\bot == 2$};
			\node[center]	at (3, 2)		(q5)	{$x_\bot == 2$};
			
			\node[center]	at (7, 6)		(q3)	{$z == [0, 0, 1, 1]$};
			\node[center]	at (7, 5)		(q4)	{$z == [0, 0, 1, 1]$};
			\node[center]	at (7, 4)		(q6)	{$y_\bot == 2$};
			\node[center] 	at (7, 3)		(q8)	{$y_b <= x_t$};
			\node[center] 	at (7, 1)		(q11)	{$y_b > x_t$};
			\node[center]	at (7, 0)		(q13)	{$x_\top == 2$};
			
			\node[center]	at (11, 4)		(q7)	{$z_\bot == 2$};
			\node[center] 	at (11, 3)		(q9)	{$z_b == y_b$};
			\node[center] 	at (11, 2)		(q10)	{$z_\bot == x_\bot$};
			\node[center]	at (11, 1)		(q12)	{$z == [0, 0, 1, 1]$};
			\node[center]	at (11, 0)		(q14)	{$z == y$};
